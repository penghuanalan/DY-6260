...
public class DetectionChannelOneActvity extends BaseActivity implements
	...
	public List<Integer> getData() {
		...
		if (buffer != null) {
			...
			if (idx != null) {
				...
				// ph将有效值之间的数组进行计算，如果存在两个下降的区间，则判断阴性，只有一个时判断其出现的位置，

				// 数组的复制，（源数组，起始位置，目标数组，起始位置，长度）截取出有用值的数组
	...
	private void value() {
				...
				if(count>=5){
					for(int j =i;j<_tValue.length - 1;j++){
						if(_tValue[j + 1] - _tValue[j] > 0){
							System.out.println("j的值"+_tValue[j]+"开始值"+_tValue[j-count]);
							index1=j;
							line=true;
							break;
						}
		...
		System.out.println("波谷1值坐标"+index1+"--波谷2值坐标"+index2+"数据的总长度"+_tValue.length+"peak和C差值"+(int)(peak-_tValue[index1])+_tValue.length+"peak和T差值"+(int)(peak-_tValue[index2]));
		if(line){
			//index1改成20原始值30
	...
	public void printTitle() throws IOException, ParseException {
		...
		if (len >= 6) {
			...
			deviceCmd = f.format(number) + space + space + etInput + space
					+ space + space+density + space + space + result + line + project
					+ line + line  + time + line
					+ "设备：胶体金读卡仪" +line+line;
			//+ line + "检测项目:"+projectName
		} else if (len < 6) {
			int newlen=6-len;
			switch(newlen){
			case 1:
				spaceNum = space+space+space+space+space+space+space;
				break;
			case 2:
				spaceNum = space+space+space+space+space+space;
				break;
			case 3:
				spaceNum = space+space+space+space+space;
				break;
			case 4:
				spaceNum = space+space+space+space;
				break;
			case 5:
				spaceNum = space+space+space;
				break;
				default :
				spaceNum=space+space+space;
			}
			deviceCmd = f.format(number) + space + space + etInput + spaceNum+ space+density + space + space
		...
		String headCmd = projectName+line+"DY-6260检测报告" + line + line;
